...
<div class="page-header d-flex justify-content-between align-items-center mb-4 p-4 rounded-3 shadow-sm bg-gradient-export">
    <div>
        <h1 class="mb-1 text-white fw-bold fs-3"><i class="fas fa-file-export me-2"></i>T?o phi?u xu?t hàng m?i</h1>
        <p class="mb-0 text-white opacity-75">Xu?t thành ph?m t? kho cho khách hàng và d?i lý</p>
    </div>
...
<div class="card border-0 shadow-sm rounded-3 mb-4">
    <div class="card-header-export p-3 border-bottom d-flex align-items-center">
        <h5 class="mb-0 text-white fw-bold"><i class="fas fa-info-circle me-2"></i>Thông tin phi?u xu?t</h5>
    </div>
        ...
        <form id="createExportForm">
            ...
            <div class="row g-4 mb-4">
                <div class="col-12">
                    ...
                    <textarea class="form-control" id="fGhichu" rows="1" placeholder="Ví d?: Xu?t hàng cho don hàng #12345..."></textarea>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h6 class="fw-bold text-export mb-0"><i class="fas fa-boxes me-2"></i>Chi ti?t danh sách hàng xu?t</h6>
            ...
            <div class="d-flex gap-3">
                <button type="submit" class="btn btn-lg btn-export-action px-5 rounded-pill shadow-sm" id="btnSubmit">
                    <i class="fas fa-check-circle me-2"></i>Xác nh?n & Luu phi?u
                </button>
...
<script>
    ...
    async function loadFormDropdowns() {
        try {
            ...
            showNotify('Không th? t?i các danh m?c l?a ch?n (Khách hàng, Kho, SP)', 'danger');
        }
    }
    ...
    function removeItemRow(btn) {
        ...
        if (rows.length > 1) {
            ...
            alert('C?n ít nh?t m?t s?n ph?m trong phi?u xu?t.');
        }
    }
    ...
    document.getElementById('createExportForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) {
                showNotify(`Ðã luu phi?u xu?t thành công! (Mã: ${data.data.id})`, 'success');
                setTimeout(() => window.location.href = 'phieu-xuat-danh-sach', 2000);
            } else {
                showNotify(data.message, 'danger');
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-check-circle me-2"></i>Xác nh?n & Luu phi?u';
            }
        } catch (error) {
